...
class BlockchainService {
  constructor() {
    ...
    this.ProxyBlockfrost = class {
      ...
      async getProtocolParameters() {
        try {
          ...
          console.log(
            "- min_fee_a:",
            params.min_fee_a,
            typeof params.min_fee_a
          );
          console.log(
            "- min_fee_b:",
            params.min_fee_b,
            typeof params.min_fee_b
          );
          console.log(
            "- key_deposit:",
            params.key_deposit,
            typeof params.key_deposit
          );
          console.log(
            "- pool_deposit:",
            params.pool_deposit,
            typeof params.pool_deposit
          );
          ...
          console.log(
            "- cost_models:",
            params.cost_models,
            typeof params.cost_models
          );
          console.log(
            "- cost_models_raw:",
            params.cost_models_raw,
            typeof params.cost_models_raw
          );
          ...
          console.log(
            "Cleaned protocol parameters (nulls replaced):",
            cleanParams
          );
          ...
          for (const field of criticalFields) {
            if (
              cleanParams[field] === undefined ||
              cleanParams[field] === null
            ) {
              console.error(
                `CRITICAL: Field ${field} is still undefined/null:`,
                cleanParams[field]
              );
            }
          }
      ...
      async submitTx(tx) {
        const txData =
          typeof tx === "string" ? tx : tx.to_hex ? tx.to_hex() : tx;
        return await self.makeBlockfrostRequest(
          "/tx/submit",
          "post",
          txData,
          null,
  ...
  getConfig() {
    const network =
      localStorage.getItem("cardano_network") || this.defaultNetwork;
  ...
  async makeBlockfrostRequest(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Backend proxy error: ${response.statusText}`
        );
      }
      ...
      throw new Error(
        error.message || "Failed to communicate with backend proxy."
      );
    }
  }
  ...
  async issueCertificate(fileHash, walletApi, fileName = "certificate.pdf") {
    try {
      ...
      try {
        console.log(
          "Initializing Lucid with direct Blockfrost (bypassing proxy for testing)..."
        );
        console.log("Service config:", serviceConfig);
        ...
        if (!lucidNetwork) {
          throw new Error(
            "Lucid network is undefined. Check network configuration."
          );
        }
        ...
        if (!apiKey) {
          throw new Error(
            "Blockfrost API key not configured. Please set REACT_APP_BLOCKFROST_API_KEY environment variable."
          );
        }
      ...
      if (!address) {
        throw new Error(
          "Could not get wallet address. Please ensure your wallet is properly connected."
        );
      }

      if (typeof address !== "string" || address.length < 10) {
        throw new Error(
          `Invalid address format received from wallet: ${address}`
        );
      }
      ...
      if (!utxos || utxos.length === 0) {
        throw new Error(
          "No UTXOs found in wallet. Please ensure your wallet has some test ADA."
        );
      }
      ...
      if (errorMessage.includes("insufficient")) {
        throw new Error(
          "Insufficient funds in wallet. Please ensure you have at least 3 ADA in your wallet."
        );
        ...
        throw new Error(
          "Blockfrost API configuration error. Please check your API key and network settings."
        );
      } else {
  ...
  async verifyCertificate(fileHash, walletAddress) {
    try {
      ...
      if (walletAddress.length > 50 && !walletAddress.startsWith("addr")) {
        console.log(
          "Address appears to be in hex format, attempting conversion..."
        );
        ...
        try {
          const serviceConfig = this.getConfig();
          const lucidNetwork =
            serviceConfig.network === "mainnet"
              ? "Mainnet"
              : serviceConfig.network === "preview"
              ? "Preview"
              : "Preprod";
      ...
      try {
        ...
        if (!apiKey) {
          return {
            valid: false,
            message:
              "Blockfrost API key not configured. Please check your environment setup.",
          };
        }
        ...
        const response = await fetch(
          `https://cardano-preprod.blockfrost.io/api/v0/addresses/${searchAddress}/transactions?order=desc&count=100`,
          {
            headers: {
              project_id: apiKey,
            },
        ...
        if (!response.ok) {
          if (response.status === 404) {
            return {
              valid: false,
              message:
                "Address not found on the blockchain. This could mean the address format is incorrect or no transactions have been made from this address.",
            };
          }
          throw new Error(
      ...
      if (transactions.length === 0) {
        return {
          valid: false,
          message:
            "No transactions found for this address. The certificate may have been issued from a different wallet or the address format may be incompatible.",
        };
      }
      ...
      for (const tx of transactions) {
        try {
          ...
          const metadataResponse = await fetch(
            `https://cardano-preprod.blockfrost.io/api/v0/txs/${tx.tx_hash}/metadata`,
            {
              headers: {
                project_id: apiKey,
              },
              ...
              if (certMetadata && certMetadata.json_metadata) {
                ...
                if (hashToCheck) {
                  ...
                  if (hashMatches) {
                    ...
                    const fileName =
                      metadataObj.file ||
                      metadataObj.filename ||
                      metadataObj.name ||
                      "Unknown File";
                    const issuer =
                      metadataObj.issuer ||
                      metadataObj.app ||
                      metadataObj.application ||
                      "Unknown Issuer";
          ...
          console.log(
            `Error checking metadata for transaction ${tx.tx_hash}:`,
            metadataError
          );
          continue;
        }
      ...
      return {
        valid: false,
        message:
          "Certificate hash not found on blockchain. The certificate may be invalid or issued on a different network.",
      };
    } catch (error) {
      ...
      if (errorMessage.includes("API key")) {
        ...
        throw new Error(
          "Address format not supported by Blockfrost API. This may be a hex-encoded address that needs conversion to bech32 format."
        );
      } else if (
        ...
        throw new Error(
          "Network error or invalid address format. Please check your wallet connection and try again."
        );
      } else {
